...
import BookFilterEditForm from './BookFilterEditForm';
...
export default function BookFilterMenu(props) {
  const { filter, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_API_HOST}/filter-product/${filter._id}`, {
        headers: {
          token: token.token,
        },
      })
    ...
    <>
      ...
      <div>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-slide-description">Bạn có chắc muốn xóa bộ lọc này?</DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <Box sx={style}>
          <BookFilterEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Menu
        open={isMenuOpen}

...
export default function BookFilterEditForm(props) {
  const { filter, closeModal, reloadData, handleSnackCreateSuccess } = props;
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, formikHelpers) => {
      (filter != null
        ? axios.put(`${process.env.REACT_APP_API_HOST}/filter-product/${filter._id}`, values, {
            headers: {
              token: token.token,
            },
          })
        : axios.post(`${process.env.REACT_APP_API_HOST}/filter-product`, values, {
            headers: {
              token: token.token,
            },
          })

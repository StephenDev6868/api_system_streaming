...
import MoodEditForm from './MoodEditForm';
...
export default function MoodMenu(props) {
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_API_HOST}/mood/${mood._id}`, {
        headers: {
          token: token.token,
        },
      })
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <DialogTitle>{mood.name}</DialogTitle>
          <DialogContent>
      ...
      >
        <Box sx={style}>
          <MoodEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Menu
        open={isMenuOpen}

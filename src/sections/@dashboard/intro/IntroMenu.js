...
export default function IntroMenu(props) {
  const { intro, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_API_HOST}/intro/${intro._id}`, {
        headers: {
          token: token.token,
        },
      })
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <DialogTitle>{intro.title}</DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-slide-description">Bạn có chắc muốn xóa intro này?</DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <Box sx={style}>
          <IntroEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Menu
        open={isMenuOpen}

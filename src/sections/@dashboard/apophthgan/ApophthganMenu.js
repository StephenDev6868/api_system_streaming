...
import ApophthganEditForm from './ApophthganEditForm';
...
export default function ApophthganMenu(props) {
  const { apophthgan, moods, reasons, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_API_HOST}/apophthgan/admin/${apophthgan._id}`, {
        headers: {
          token: token.token,
        },
      })
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <DialogTitle>{apophthgan.title}</DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-slide-description">
              Bạn có chắc muốn xóa danh ngôn này?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <Box sx={style}>
          <ApophthganEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Menu
        open={isMenuOpen}

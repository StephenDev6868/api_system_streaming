...
export default function CategoryMenu(props) {
  const { category, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .post(`${process.env.REACT_APP_API_HOST}/category/admin/delete/${category._id}`, null, {
        headers: {
          token: token.token,
        },
      })
  ...
  return (
    <>
      ...
      >
        <Box sx={style}>
          <CategoryEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Menu
        open={isMenuOpen}

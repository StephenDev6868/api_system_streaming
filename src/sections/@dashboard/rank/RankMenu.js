...
import BannerEditForm from './RankEditForm';
...
export default function RankMenu(props) {
  const { rank, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_API_HOST}/membership/${rank._id}`, {
        headers: {
          token: token.token,
        },
      })
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <DialogTitle>{rank.name}</DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-slide-description">Bạn có chắc muốn xóa hạng này?</DialogContentText>
          </DialogContent>
          <DialogActions>

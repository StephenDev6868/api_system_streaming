...
import IssueTypeEditForm from './IssueTypeEditForm';
...
export default function IssueTypeMenu(props) {
  const { issueType, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_API_HOST}/advisory/issue-type/${issueType._id}`, {
        headers: {
          token: token.token,
        },
      })
    ...
    <>
      ...
      <div>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-slide-description">Bạn có chắc muốn xóa vấn đề này?</DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <Box sx={style}>
          <IssueTypeEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Menu
        open={isMenuOpen}

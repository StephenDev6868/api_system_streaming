...
import AppellationEditForm from './AppellationEditForm';
...
export default function AppellationMenu(props) {
  const { appellation, reloadData } = props;
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_API_HOST}/appellation/${appellation._id}`, {
        headers: {
          token: token.token,
        },
      })
  ...
  return (
    <>
      ...
      <div>
        ...
        >
          <DialogTitle>{appellation.name}</DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-slide-description">
              Bạn có chắc muốn xóa chức danh này?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <Box sx={style}>
          <AppellationEditForm {...props} closeModal={handleCloseEditDialog} />
        </Box>
      </Modal>
      <Menu
        open={isMenuOpen}
